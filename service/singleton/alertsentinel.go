...
func collectCycleTransferQueries(checkAt time.Time) []cycleTransferQuery {
	...
	for _, alert := range Alerts {
		...
		for ruleIdx := range alert.Rules {
			...
			for _, server := range ServerList {
				...
				if rule.NextTransferAt[server.ID].After(checkAt) {
					continue
				}
				var sel string
				switch rule.Type {
...
func checkStatus() {
	...
	ServerLock.RLock()
	AlertsLock.Lock()
	queries := collectCycleTransferQueries(checkAt)
	AlertsLock.Unlock()
	ServerLock.RUnlock()
	...
	for _, alert := range Alerts {
		...
		for _, server := range ServerList {
			...
			alertsStore[alert.ID][server.ID] = append(alertsStore[alert.
				ID][server.ID], alert.Snapshot(AlertsCycleTransferStatsStore[alert.ID], server, cycleFromDB, checkAt))
			// 发送通知，分为触发报警和恢复通知
