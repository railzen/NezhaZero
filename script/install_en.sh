...
NZ_MAIN_DEFAULT_VERSION="v0.20.22"
NZ_AGENT_DEFAULT_VERSION="v0.20.22"
...
pre_install_agent_bin() {
    ...
    # _version=$(curl -m 10 -sL "https://api.github.com/repos/railzen/nezha-zero/releases/latest" | grep "tag_name" | head -n 1 | awk -F ":" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')

    # if [ -z "$_version" ]; then
    #     err "Fail to obtain Agent version, please check if the network can link https://api.github.com/repos/railzen/nezha-zero/releases/latest"
    #     return 1
    # else
...
discover_agent() {
    ...
    _cmd="sudo ${NZ_AGENT_PATH}/nezha-agent service install -s $nz_grpc_host:$nz_grpc_port --auto-discover $nz_discover_secret $args >/dev/null 2>&1"
